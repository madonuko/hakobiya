...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        manager.drop_table(Table::drop().table(HoldEvent::Table).to_owned()).await?;
        manager.drop_table(Table::drop().table(JoinSubEvent::Table).to_owned()).await?;
        manager.drop_table(Table::drop().table(JoinEvent::Table).to_owned()).await?;
        manager.drop_table(Table::drop().table(SubEvent::Table).to_owned()).await?;
